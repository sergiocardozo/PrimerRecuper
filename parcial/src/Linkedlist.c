...
static int addNode(LinkedList* this, int nodeIndex,void* pElement)
{
    ...
    if(this != NULL && nodeIndex >= 0 && nodeIndex <= ll_len(this))
    {
    	...
    	{
    		newNode->pNextNode = this->pFirstNode;//el primer nodo va a hacer que el nodo que se creo apunte al siguiente nodo
    		this->pFirstNode = newNode;//el nuevo nodo va a ser el primer nodo
    		this->size ++;//y va a aumentar la lista en 1
    		returnAux = 0;
    	}
    	else
    	{
    		pActualNode = getNode(this,nodeIndex - 1); //obtengo el nodo anterior y se lo asigna al nodo actual
    		newNode->pNextNode = pActualNode->pNextNode; //el nodo obtenido va a apuntar al nuevo nodo y este apunta al siguiente
    		pActualNode->pNextNode = newNode; //el nuevo nodo se va a guardar en la posicion indicada
...
{
    LinkedList* cloneArray = NULL; //inicializo la lista en null o vacia
    int i;
    Node* pNode; // puntero al nodo
    if(this!=NULL && pFunc!=NULL) // verifico que la lista tenga elemento y la funcion criterio no este vacia
    {
    	cloneArray = ll_newLinkedList(); // le asigno a la lista el nuevo espacio de memoria dinamica
    	pNode = this->pFirstNode; // a node le asigno la direccion de memoria del primer nodo
		for(i=0;i<this->size;i++) // recorro el largo de la lista
		{
			if(pFunc(pNode->pElement)) // si la funcion criterio contiene los elementos a a los que apunta el nodo
			{
				ll_add(cloneArray,pNode->pElement);//agrega los elementos a la nueva lista
			}
			//if(pNode!=NULL)
				pNode = pNode->pNextNode;// ahora el nodo apunta al proximo nodo
		}
    }

    return cloneArray;//retorna la lista
}
